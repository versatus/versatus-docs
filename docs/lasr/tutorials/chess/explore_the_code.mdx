...
The `create` method is required in every program created with LASR, as it's executed during deployment. The `create` method utilizes parameters specified in the [`lasrctl deploy`](/lasr/tutorials/chess/deploy#deploy-chess) command to set up the program. For LASR Chess, this method creates an NFT.
