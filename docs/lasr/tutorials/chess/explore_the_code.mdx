...
id: explore_the_code
slug: /tutorials/chess/explore_the_code
description: ''
tags: []
---
...
LASR Chess program is composed of some core functions that define how it works. This page explores all these functionalities LASR Chess has to offer, presenting you details about each method available in the `lasr-chess.ts` file found in the [cloned project](/tutorials/chess/deploy).
...
The `create` method is required in every program created with LASR, as it's executed during deployment. The `create` method utilizes parameters specified in the [`lasrctl deploy`](/tutorials/chess/deploy#deploy-chess) command to set up the program. For LASR Chess, this method creates an NFT.
