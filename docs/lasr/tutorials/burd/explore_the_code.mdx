...
id: explore_the_code
slug: /tutorials/burd/explore_the_code
description: ''
tags: []
---
...
BURD program is composed of some core functions that define how it works. This page explores all these functionalities BURD has to offer, presenting you details about each method available in the `burd.ts` file found in the [cloned project](/tutorials/burd/deploy).
...
The `create` method is required in every program created with LASR, as it's executed during deployment. The `create` method utilizes parameters specified in the [`lasrctl deploy`](/tutorials/burd/deploy#deploy-burd) command to set up the program. For BURD, this method creates an NFT.
