...
slug: /getting_started/creating_your_first_program
description: ''
tags: []
---
...
With the project setted up, you will now initialize a new LASR project. You can use a variety of templates to perform this. In this tutorial, you will use the `hello-lasr`:
...
:::info
For more details about the available templates, access the [Templates](/core_concepts/templates) page. 
:::

The `init` command will generate an example program named example-program.ts, located at the root of your project. In addition, it also generates a folder containing example JSON inputs for each method within the inputs directory. These JSON files have examples of what the LASR protocol will send your program. For more information about how the protocol works, access the [LASR Protocol](/core_concepts/lasr_protocol) page.

### Set up your Wallet Keys

When the `lasrctl` command `init` ran in [Step 2](#start-the-project), it also generated a `.lars/wallet/keypairs.json` file in the project. These addresses are used to connect your account, created in the [Creating an Account](/getting_started/creating_an_account) page to the program you will deploy. You need to replace `address` and `secret_key` with your own:
...
<MediaWrapper>
![](../../public/get_started/hello_world/playground-keypairs2.png)
</MediaWrapper>
...
The instructs `lasrctl` to iterate through each JSON file in the specified `./example-program-inputs` directory, feeding them into your program. The results of these operations are then displayed in the terminal. You will observe a series of messages indicating the validation status of your program's output.  Following is an example of what you can expect to see:
...
```bash copy
npx lasrctl deploy -b example-program -s HELLO_WORLD -p MY_FIRST_PROGRAM
```
...
Where:
- `-s`: The symbol for the program.
- `-p`: The name for the program.

Access the [Deploy Configuration](/lasr_guides/deploy_configuration) page to learn more about the available parameters you can use when deploying your program.
...
- **LASR CLI**: It's installed together with  [@versatus/versatus-javascript package](https://www.npmjs.com/package/@versatus/versatus-javascript) and enables you to interact and test your program using your terminal. To learn more about the available CLI features, access the [LASR CLI](/lasr_guides/cli) page.
...
```
<MediaWrapper>
![](../../public/get_started/hello_world/hello_lasr_gif.gif)
</MediaWrapper>
...
- [Blockchain Fundamentals](/core_concepts/blockchain_fundamentals)
- [LASR Protocol](/core_concepts/lasr_protocol)
- [Templates](/core_concepts/templates)

In case you want to explore the LASR features, you can access one of the following options:

- [LASR Deploy Examples](/tutorials)
- [LASR CLI](/lasr_guides/cli)
- [Deploy Process](/lasr_guides/deploy_process)
- [Deploy Configurations](/lasr_guides/deploy_configuration)
